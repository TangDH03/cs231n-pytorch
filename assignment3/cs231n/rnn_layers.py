...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    h = prev_h.shape[1]
    new_h = x.dot(Wx)+prev_h.dot(Wh)+b
    ai = new_h[:,:h]
    af = new_h[:,h:h*2]
    ao = new_h[:,2*h:3*h]
    ag = new_h[:,3*h:4*h]
    ...
    cache = (new_h, i, f, o, g, next_c, next_h, x, prev_h, prev_c, Wx, Wh)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache=[]
    for i in range(T):
      h0,prev_c,step_cache = lstm_step_forward(x[:,i,:],h0,prev_c,Wx,Wh,b)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
      if(i==T-1):
        ...
        dnext_h += dh[:,i,:]
      dx,dnext_h,dprev_c,dWx,dWh,db = lstm_step_backward(dnext_h,dprev_c,cache[i])
      if(i==T-1):
        dX = np.zeros([N,T,dWx.shape[0]])
